...
```{r}
 ...
 wf_list <- list(w01=wf01,w02=wf02)
...
mape_df <- augment_data_list |>
  purrr::map_df(\(x) x|> mape(truth = num_sold, estimate=.pred))
mape_df |>print()
```
```{r}
plts <-
  ...
  purrr::map(\(df) 
             df|>
               ggplot(aes(x=num_sold, y=.resid_pct)) +
               geom_hex() )
plts|>map(\(p) print(p))
```
...
```{r}
augmented_data <- 
  augment(fit_wf, new_data =train_data) |>
  mutate(.resid_pct=.resid/num_sold)  
# |> select(true=num_sold, .pred, .resid, .resid_pct)
augmented_data|>mape(truth = num_sold, estimate=.pred) |>print()
augmented_data |> 
  ggplot(aes(x=num_sold, y=.resid_pct)) +
  geom_hex()
```
